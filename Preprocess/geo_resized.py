...
def resize_rasters(image_dir, mask_dir, resized_image_dir, resized_mask_dir, dst_res=None):
    """
    ...
    Args:
        ...
        dst_res (float, optional): Resolución deseada en grados por píxel. Si es None, no se aplica redimensionamiento.
    ...
    def process_raster(src_path, dst_path, resampling_method, dst_res):
        """Procesa un raster para redimensionarlo."""
        with rasterio.open(src_path) as src:
            transform = src.transform
            crs = src.crs
            current_res = transform.a  # Resolución original

            if dst_res is None:
                dst_res = current_res  # Mantener la resolución original
